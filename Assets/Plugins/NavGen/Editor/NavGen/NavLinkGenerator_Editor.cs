...
namespace idbrii.navgen
{
    ...
    public class NavLinkGenerator_Editor : Editor
    {
        ...
        [SerializeField] public bool m_UseOffMeshLink;
        [SerializeField] bool m_AttachDebugToLinks;
        [SerializeField] bool m_ShowCreatedLinks;
        [SerializeField] List<GameObject> m_CreatedLinks = new List<GameObject>();
        ...
        public override void OnInspectorGUI()
        {
            ...
            m_UseOffMeshLink = EditorGUILayout.Toggle("Use OffMeshLink", m_UseOffMeshLink);
            ...
            using (new GUILayout.HorizontalScope())
            {
                ...
                {
                    var count = GenMeshLinks(gen);
                    if (m_UseOffMeshLink)
                        Debug.Log($"Baked OffMeshLinks:" + count);
                    else
                        Debug.Log($"Baked NavMeshLinks:" + count);
                }
        ...
        int GenMeshLinks(NavLinkGenerator gen)
        {
            var tri = NavMesh.CalculateTriangulation();
            var edge_list = CreateEdges(tri);
            foreach (var edge in edge_list)
            ...
            foreach (var edge in edge_list)
            {
                ...
                var link = CreateLink(parent, gen, edge, mid, fwd,m_UseOffMeshLink);
                if (link != null)
        ...
        GameObject CreateLink(Transform parent, NavLinkGenerator gen, NavEdge edge, Vector3 mid, Vector3 fwd,bool useOffMeshLink)
        {
            RaycastHit phys_hit;
            RaycastHit ignored;
            NavMeshHit nav_hit;
            ...
            for (int i = 0; i < gen.m_Steps; ++i)
            {
                ...
                if (hit)
                {
                    ...
                    if (hit)
                    {
                        ...
                        if (useOffMeshLink)
                        {
                            var t = PrefabUtility.InstantiatePrefab(gen.OffMeshLinkPrefab, parent.gameObject.scene) as Transform;
                            Debug.Assert(t != null, $"Failed to instantiate {gen.OffMeshLinkPrefab}");
                            t.SetParent(parent);
                            ...
                            Undo.RegisterCompleteObjectUndo(link.gameObject, "Create OffMeshLink");
                        }
                        else {
        ...
        void RemoveLinks()
        {
            var nav_links = NavEdUtil.GetNamedRoot(k_LinkRootName).GetComponentsInChildren<Transform>();
