...
namespace idbrii.navgen.runtime
{
    ...
    public delegate void OnLinkBuildFinashed();
    ...
    public class NavLinkGenerator_Runtime : MonoBehaviour
    {
        ...
        //public OffMeshLink OffMeshLink_Prefab;
        ...
        public Transform LinksRoot
        {
            get
            {
                if (_LinksRoot == null)
                {
                    ...
                    if(m_UseOffMeshLink)
                        linkroot.name = "OffMeshLinks";
                    else
                        linkroot.name = "NavMeshLinks";
                    _LinksRoot = linkroot.transform;
                }
                return _LinksRoot;
            }
        }
        ...
        public static OnLinkBuildFinashed RebuildNavMesh_CallBack;
        public static bool HasNavMesh {get { return main != null && main.isActiveAndEnabled && main.navMeshSurface != null && main.navMeshSurface.navMeshData != null;}}

        //全局引用
        public static NavLinkGenerator_Runtime main;
        NavLinkGenerator_Runtime() {
            main = this;
        }
        public static void EnsureInit() {
            //实例化自己的单例
            if (NavLinkGenerator_Runtime.main == null || !NavLinkGenerator_Runtime.main.isActiveAndEnabled)//为空or有残留
                Instantiate(Resources.Load<GameObject>("Prefab/NavMeshManager"));
        }

        void Awake()
        {
            navMeshSurface = GetComponent<NavMeshSurface>();
            Debug.Assert(navMeshSurface != null);
        }
        ...
        public static void RebuildAll() {
            EnsureInit();
            main.navMeshSurface.BuildNavMesh();
            main.RebuildLinks();
        }
        ...
        public void RebuildLinks()
        {
            ...
            if(RebuildNavMesh_CallBack !=null)
                RebuildNavMesh_CallBack();
        ...
        GameObject CreateLink(Transform parent, NavEdge edge, Vector3 mid, Vector3 fwd,bool useOffMeshLink)
        {
            ...
            for (int i = 0; i < m_Steps; ++i)
            {
                ...
                //Debug.DrawLine(mid, top, hit ? ground_found : ground_missing, 1000);
                //Debug.DrawLine(top, down, hit ? ground_found : ground_missing, 1000);
                if (hit)
                {
                    ...
                    //Debug.DrawLine(phys_hit.point, nav_hit.position, hit ? navmesh_found : navmesh_missing, 1000);
                    if (hit)
                    {
                        ...
                        //Debug.DrawLine(transit_start, transit_end, hit ? traverse_clear : traverse_hit, 1000    );
                        if (hit)
                        {
